...
from msgfusion.config_shim import runtime_config as args
import cv2    
...
#./images/IV_images
#./images/IV_images
def getTranImagePatches(prepath): # Split original infrared (IR) and visible (VIS) images into fixed-size patches for training-time data augmentation.
    patchesIR = [];
    patchesVIS = [];
    picIdx = 0;
...
def gradient2(x): # Compute gradient maps with different convolution kernels to extract edges or texture cues.
    dim = x.shape;
    if (args.cuda):
...
def sumPatch(x,k): # Image smoothing
    dim = x.shape;
    if (args.cuda):
        x = x.cuda(int(args.device));
...
def enumerate_training_sample_ids():
    """Enumerate patch ids ``\"1\"..str(trainNumber)`` (inclusive)."""
    ordered = []
    for k in range(1, args.trainNumber + 1):
...
def shuffle_trim_to_batches(image_path, BATCH_SIZE, num_imgs=None):
    """Shuffle indices and trim tail so len % BATCH_SIZE == 0."""
    if num_imgs is None:
        num_imgs = len(image_path)
    remainder = num_imgs % BATCH_SIZE
    print("BATCH SIZE %d." % BATCH_SIZE)
    print("Train images number %d." % num_imgs)
    print("Train images samples %s." % str(num_imgs / BATCH_SIZE))
    if remainder > 0:
        print("Train set has been trimmed %d samples...\n" % remainder)
        image_path = image_path[:-remainder]
    num_imgs -= remainder
    working_list = image_path[:num_imgs]

    random.shuffle(working_list)
    batches = int(len(working_list) // BATCH_SIZE)
    return working_list, batches
...
def load_grayscale_training_tensor(path):
    images = []
    mode = 'L';
    ...
    images.append(image)
    images = np.stack(images, axis=0)
    #images = torch.from_numpy(images).float(); # 2025.4.28 (53) updated
...
# colormap
def colormap(): # Define and return a custom linear colormap for visualizing gradient maps or fusion outputs.
    return mpl.colors.LinearSegmentedColormap.from_list('cmap', ['#FFFFFF', '#98F5FF', '#00FF00', '#FFFF00','#FF0000', '#8B0000'], 256)

def load_mask(path):
    img = Image.open(path).convert('L')       # Grayscale
    mask = np.array(img, dtype=np.float32)    # 0~255
    mask = (mask > 127).astype(np.float32)    # Binarization
    mask = torch.from_numpy(mask).unsqueeze(0).unsqueeze(0) / 1.0 # 2025.4.28 (53) updated
    #mask = torch.as_tensor(mask).unsqueeze(0).unsqueeze(0)
